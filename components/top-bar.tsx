...
export default function TopBar({ onMenuClick, instagramUsername, instagramAccountCount }: TopBarProps) {
  ...
  return <header className="sticky top-0 z-30 flex h-16 shrink-0 items-center justify-between gap-3 border-b border-border/80 bg-white/92 px-3 backdrop-blur-xl sm:px-6 lg:h-20 lg:px-8">
    <div className="flex min-w-0 items-center gap-2.5 sm:gap-4">
      <button type="button" onClick={onMenuClick} className="grid min-h-11 min-w-11 shrink-0 place-items-center rounded-xl border border-border bg-white text-muted shadow-sm hover:bg-surface-hover hover:text-foreground lg:hidden" aria-label="Open navigation">
    ...
    {instagramAccountCount > 0 ? <div className="flex min-h-10 shrink-0 items-center gap-2 rounded-full border border-border bg-surface-hover/70 px-2.5 sm:px-3">
      <span className="relative grid h-7 w-7 place-items-center rounded-full bg-gradient-to-br from-[#7668ed] to-[#4b4fc4] text-white"><Icon name="instagram" size={14} /><span className="absolute -right-0.5 -top-0.5 h-2.5 w-2.5 rounded-full border-2 border-white bg-success" /></span>
