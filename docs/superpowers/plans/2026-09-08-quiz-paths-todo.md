...
Etapy 1–12 są zaimplementowane. `npm test`: 280 testów PASS, w tym rzeczywisty izolowany PostgreSQL i adapter Neon. Pełny test podpisanego webhooka → R2 → Queue → Jobs → PostgreSQL z atrapą wyłącznie Meta przechodzi, łącznie z błędnym e-mailem i starym przyciskiem. Test nie oznacza wysyłki na prawdziwym Instagramie.
