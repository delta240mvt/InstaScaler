...
export default function OverviewPage() {
  ...
  useEffect(() => { api.accounts.list().then(({ data }) => { setAccounts(data.instagramAccounts); setSelected(data.selectedInstagramAccountId ?? ""); }).finally(() => setLoading(false)); }, []);
  useEffect(() => {
    if (!selected) return;
    api.accounts.overview(selected).then(({ data }) => {
