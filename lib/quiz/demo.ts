import type { QuizGraph, QuizTrigger } from "./contracts";

export function createDemoGraph(trigger: QuizTrigger = "comment"): QuizGraph {
  return { schemaVersion: 1, qualification: { mode: "any", rules: [{ kind: "email" }, { kind: "interest" }] }, nodes: [
    { id: "start", label: trigger === "dm" ? "Słowo w DM" : "Komentarz START", x: 40, y: 40, type: "start", trigger, keyword: "START", postIds: [], allPosts: trigger === "comment", text: "Chcesz przejść krótki quiz? Kliknij Zaczynamy. Możesz zakończyć rozmowę, pisząc STOP.", cta: "Zaczynamy", next: "wybor" },
